...
class Order2Fragmentkopia : Fragment() {
    ...
    private fun refresh() {
        ...
        city?.let {
            freeDelivery = it.freePrice.isNotEmpty() && it.freePrice != "-1" && app.cart.totalPrice.toFloat() >= it.freePrice.toFloat()

            if (freeDelivery) {
                payTypeButtons.visibility = View.GONE
                var text = "Cena dostawy: darmowa\n"
                if (it.freePrice.isNotEmpty() && it.freePrice != "-1") text += String.format("Darmowa dostawa: dla zamówienia powyżej: %.2f zł", it.freePrice.toFloat())
