...
class ProductCouponFragment() : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        product.additions.forEach { id ->
            app.productAdditions.firstOrNull { it.id==id }?.let {
                additions.add( it.name + "   +" + String.format("%s pkt", it.pkt) )
            }
        }

        addToCartButton.setOnClickListener {
            Product.showAddModal(product, true)
        }

        App.instance.storage.loadProductImage(bannerImage, product)

        pktText.text = String.format("%s pkt", product.pkt)
